...
export const BookDL = {
    ...
    tags: new DataLoader(async (bookIDs: readonly string[]) => {
        ...
        return bookIDs.map(id => tags.filter(tag => tagsForBookObj.some(item => item.bookID?.toString() === id.toString() && item?.tagID?.toString() === tag._id.toString())));
    }),
    ...
    readDate: new DataLoader(async (bookIDs: readonly string[]) => {
        ...
        return bookIDs.map(id => readDates.filter(item => item.bookID?.toString() === id.toString()));
    }),
    ...
    additionalMedia: new DataLoader(async (bookIDs: readonly string[]) => {
        ...
        return bookIDs.map(id => {
            const mediaForBook = groupedMedia.filter(item => item._id.bookID.toString() === id.toString());
            return {
