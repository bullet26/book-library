...
export const AuthorDL = {
  books: new DataLoader(async (authorIDs: readonly string[]) => {
    ...
    return authorIDs.map(
      (id) =>
        books
          .filter((item) => item.authorID?.toString() === id.toString())
          .map((book) => book.toObject()), // TODO подумать как лучше сделать с toObject()
    )
  }),
  ...
  series: new DataLoader(async (authorIDs: readonly string[]) => {
    ...
    return authorIDs.map((id) =>
      series.filter((item) => item.authorID?.toString() === id.toString()),
    )
  }),
  ...
  booksWithoutSeries: new DataLoader(async (authorIDs: readonly string[]) => {
    ...
    return authorIDs.map((id) =>
      books.filter((item) => item.authorID?.toString() === id.toString()),
    )
  }),
}
